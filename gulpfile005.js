...
gulp.task('default', [ 'js-linting-compiling' ]) ; 

gulp.task('hello-world', function(){
	console.log("Hello world."); 
}) ; 

gulp.task('js-linting-compiling', ['hello-world'], function() {
	console.log("Let's do some work.");
	return gulp.src('script/lib/*.js') // read all of the files that are in
										// script/lib with a .js extension
	.pipe(jshint()) // run their contents through jshint
	.pipe(jshint.reporter('default')) // report any findings from jshint
	.pipe(concat('all.js')) // concatenate all of the file contents into a file
							// titled 'all.js'
